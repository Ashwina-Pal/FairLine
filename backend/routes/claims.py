...
async def create_claim(
    ...
    receipt_image: UploadFile | None = File(None, description="Receipt image file upload (PNG/JPG)"),
):
    """
    ...
    manual_input = ManualInput(amount=amount, category=category, date=date, cost_center=cost_center)
    ...
    def write_audit_log(actor: str, action: str, details: dict | None = None):
    ...
    write_audit_log(actor="EMPLOYEE", action="submitted", details=manual_input.model_dump())
    ...
    if receipt_image:
        try:
            ...
            write_audit_log(actor="CUSTOM_AGENT", action="extracted", details=extracted_data.model_dump())

        except Exception as e:
            print(f"Receipt extraction failed: {e}. Defaulting metadata values.")
            extracted_data = ExtractedData(
                merchant="Extraction Failed", total_amount=0.0, date="N/A", confidence_score=0.0
            )
            claim_ref.update({"extracted_data": extracted_data.model_dump()})
            write_audit_log(actor="CUSTOM_AGENT", action="extraction_failed", details={"error": str(e)})
    else:
        # Default empty extraction metadata if no file was uploaded
        extracted_data = ExtractedData(merchant="N/A", total_amount=0.0, date="N/A", confidence_score=0.0)
        claim_ref.update({"extracted_data": extracted_data.model_dump()})
    ...
    write_audit_log(actor="CUSTOM_SKILL", action="policy_checked", details=evidence_packet.model_dump())
    ...
    if not evidence_packet.is_escalated:
        ...
        write_audit_log(actor="CUSTOM_SKILL", action="auto-cleared")
    else:
        status = "ESCALATED"
        claim_ref.update({"status": status})
        write_audit_log(
            actor="CUSTOM_SKILL", action="flagged", details={"flagged_rule_ids": evidence_packet.flagged_rule_ids}
        )
